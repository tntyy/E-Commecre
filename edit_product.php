...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update sản phẩm
    $stmtUpdate = $db->prepare("UPDATE products 
                                SET title = ?, price = ?, catid = ?, image = ?, status = ?
                                WHERE id = ?");
    $stmtUpdate->execute([$title, $price, $catid, $image, $status, $id]);

    // Redirect về xoasua.php theo category
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select name="catid" class="form-control" required>
                ...
                <?php foreach ($categories as $c): ?>
                    <option value="<?= $c['id'] ?>" <?= ($c['id'] == $product['catid']) ? 'selected' : '' ?>>
                        <?= htmlspecialchars($c['name']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div class="mb-3">
            ...
            <?php if ($product['image']): ?>
                <img src="images/<?= $product['image'] ?>" alt="" style="width:150px; height:auto; margin-bottom:10px; display:block;">
            <?php else: ?>
                No image
            <?php endif; ?>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $email = $_POST['email'];
    $pass = $_POST['pass']; // có thể hash nếu cần
    $date_expires = $_POST['date_expires'];

    // Update vào DB
    $stmtUpdate = $db->prepare("UPDATE users SET type = ?, username = ?, email = ?, pass = ?, date_expires = ?, date_modified = NOW() WHERE id = ?");
    $stmtUpdate->execute([$type, $username, $email, $pass, $date_expires, $id]);
...
<div class="container mt-5">
    <h2>Edit Admin: <?= htmlspecialchars($admin['username']) ?></h2>
    <form action="admin.php" method="POST">
        <div class="mb-3">
            <label>Type</label>
            <input type="text" name="type" class="form-control" value="<?= htmlspecialchars($admin['type']) ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="pass" class="form-control" value="<?= htmlspecialchars($admin['pass']) ?>" required>
        </div>

        <div class="mb-3">
            <label>Date Expires</label>
            <input type="date" name="date_expires" class="form-control" value="<?= htmlspecialchars($admin['date_expires']) ?>">
        </div>

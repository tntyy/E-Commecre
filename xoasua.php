...
// Lấy danh sách category để menu
$stmtCate = $db->query("SELECT id, name FROM category");
$categories = $stmtCate->fetchAll(PDO::FETCH_ASSOC);

// Lấy category_id từ GET (nếu có)
$category_id = isset($_GET['category_id']) ? (int)$_GET['category_id'] : 0;

// Lấy tên category (để hiển thị tiêu đề bảng)
if ($category_id > 0) {
    $stmtCat = $db->prepare("SELECT name FROM category WHERE id=?");
    $stmtCat->execute([$category_id]);
    $cat = $stmtCat->fetch(PDO::FETCH_ASSOC);
    $category_name = $cat ? $cat['name'] : 'Unknown';
} else {
    $category_name = 'All Cars';
}
...
if ($category_id > 0) {
    ...
    $stmt = $db->query("SELECT p.*, c.name AS cat_name 
                        FROM products p 
                        JOIN category c ON p.catid = c.id");
    $products = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<div class="container mt-5">
    ...
    <h2>Products in Category: <?= htmlspecialchars($category_name) ?></h2>
<a href="add_product.php<?= $category_id > 0 ? '?category_id='.$category_id : '' ?>" 
   class="btn add-admin-btn mb-3">+ Add Product</a>
        ...
        <?php if(!empty($products)): ?>
            <?php foreach($products as $p): ?>
                <tr>
                    ...
                    <td><img src="images/<?= $p['image'] ?>" style="width:80px; height:50px; object-fit:cover;"></td>
